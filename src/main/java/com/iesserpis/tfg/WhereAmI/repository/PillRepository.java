...
public interface PillRepository extends JpaRepository<Pill, Integer> {
    ...
    @Query("SELECT new com.iesserpis.tfg.WhereAmI.responesAPI.PillResponse(pk.name, p.effect, p.polarity) FROM Pickup pk JOIN Pill p ON p.id = pk.id")
    List<PillResponse> findAllPills();

    @Query("SELECT new com.iesserpis.tfg.WhereAmI.responesAPI.PillResponse(pk.name, p.effect, p.polarity) FROM Pickup pk JOIN Pill p ON p.id = pk.id where p.id = :idpill")
